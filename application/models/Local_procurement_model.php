...
class Local_procurement_model extends CI_Model {
    ...
    /*$this->db->select("
            material_sourcing_list.msid as msid,
            material_sourcing_list.created_date as request_date,
            material_sourcing_list.quantity as quantity,
            material_sourcing_list.uom as uom,
            CONCAT(employees.last_name, ',', employees.first_name , ' ', employees.middle_name) as requestor_name,
            material_sourcing.date_required as date_required,
            LAST(material_approval_list.role_status) as role_status,
            LAST(material_approval_list.status) as status
        ");
        $this->db->from('blaine_local_procurement.material_sourcing_list');
        $this->db->join('blaine_local_procurement.material_sourcing', 'blaine_local_procurement.material_sourcing.msid = blaine_local_procurement.material_sourcing_list.msid','left');
        $this->db->join('blaine_intranet.users', 'blaine_intranet.users.username = blaine_local_procurement.material_sourcing_list.created_by','left');
        $this->db->join('blaine_intranet.employees', 'blaine_intranet.employees.employee_number = blaine_intranet.users.employee_number','left');
        $this->db->join('blaine_intranet.employees', 'blaine_intranet.employees.employee_number = blaine_intranet.users.employee_number','left');
        $this->db->where('blaine_local_procurement.material_sourcing.is_active', 1);*/
